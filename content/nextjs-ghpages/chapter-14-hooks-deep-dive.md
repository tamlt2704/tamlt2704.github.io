...
[← Chapter 13: React Mental Model](/blog/nextjs-ghpages/chapter-13-react-mental-model) | [Chapter 15: TypeScript for This Project →](/blog/nextjs-ghpages/chapter-15-typescript)
...
function MarkdownCode({ children, className }) {
  ...
  return <SyntaxHighlighter style={isDark ? oneDark : oneLight}>{children}</SyntaxHighlighter>;
}
```
...
function CodePlayground({ initialCode }) {
  ...
  return <textarea ref={textareaRef} defaultValue={initialCode} />;
}
```
