...
[← Chapter 12: JS Essentials](/blog/nextjs-ghpages/chapter-12-js-essentials) | [Chapter 14: Hooks Deep Dive →](/blog/nextjs-ghpages/chapter-14-hooks-deep-dive)
...
function Button({ label, onClick, variant = "primary" }) {
  ...
  //     JSX ↓
  return (
    <button className={`btn btn-${variant}`} onClick={handleClick} disabled={loading}>
      {loading ? "..." : label}
    </button>
  );
}
...
Why? React tracks hooks by their _order_ in the component. If you put a hook inside an `if`, the order changes between renders, and React loses track.
...
setTodos(todos.map((todo) => (todo.text === "Learn React" ? { ...todo, done: true } : todo)));
```
