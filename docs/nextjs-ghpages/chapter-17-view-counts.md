...
[← Chapter 16: Supabase Setup](chapter-16-supabase-setup.md) | [Chapter 18: Auth & User Progress →](chapter-18-auth-progress.md)
...
<div className="flex items-center gap-3 text-sm text-gray-500 mb-6">
  <span>{seriesTitle} / {fileSlug}</span>
  <span>·</span>
      ...
      <ul className="space-y-2">
        {posts.map((post) => (
          <li key={post.slug}>
            <Link
              href={`/blog/${post.slug}`}
              className="text-sm text-gray-700 dark:text-gray-300 hover:text-teal-600"
            >
              {post.slug.split("/").pop()?.replace(/-/g, " ")}
            </Link>
            <span className="text-xs text-gray-400 ml-2">
              {post.count.toLocaleString()} views
            </span>
          </li>
        ))}
      </ul>
