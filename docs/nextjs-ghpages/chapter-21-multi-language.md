...
[← Chapter 20: Monetization](chapter-20-monetization.md)
...
function Quiz({ question, options, answer }: QuizProps) {
  ...
  // ...
  {revealed && (
    <p>{isCorrect ? t("quiz.correct") : `${t("quiz.wrong")} ${correctLetter}.`}</p>
  )}
}
```
...
export default async function CourseIndex({ params }: Props) {
  ...
  const courses = fs.existsSync(base)
    ? fs.readdirSync(base, { withFileTypes: true })
        .filter((d) => d.isDirectory())
        .map((d) => {
          const chapters = fs.readdirSync(path.join(base, d.name))
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-12">
      <h1 className="text-3xl font-bold mb-8">{titles[lang]}</h1>
      <div className="grid gap-4">
        {courses.map((course) => (
          <Link
            key={course.name}
            href={`/blog/${lang}/${course.name}`}
            className="block p-5 rounded-lg border hover:border-teal-400 transition"
          >
            <h2 className="font-semibold text-lg">
              {course.name.replace(/-/g, " ").replace(/\b\w/g, c => c.toUpperCase())}
            </h2>
            <p className="text-sm text-gray-500 mt-1">{course.chapters} chapters</p>
          </Link>
        ))}
      </div>
    </main>
