...
[← Chapter 22: Gated Resources](chapter-22-gated-resources.md)
    ...
    <header className="sticky top-0 z-50 border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md">
      <nav className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="font-semibold text-gray-900 dark:text-white group-hover:text-teal-600 transition hidden sm:inline">
            ACode
          </span>
        </Link>
...
export function MobileMenu() {
  ...
  return (
    <div className="md:hidden">
      ...
      {open && (
        <div className="absolute top-14 left-0 right-0 bg-white dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800 p-4 space-y-3">
          <MobileLink href="/blog" onClick={() => setOpen(false)}>Courses</MobileLink>
...
function MobileLink({ href, children, onClick }: { href: string; children: React.ReactNode; onClick: () => void }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block text-base text-gray-700 dark:text-gray-300 hover:text-teal-600 py-2"
    >
      {children}
    </Link>
  );
...
export function CourseCard({ title, description, chapters, href, icon, color, tags }: Props) {
  return (
    <Link href={href} className="group block">
      <div className="relative overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 transition-all duration-200 hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-lg hover:-translate-y-0.5">
        {/* Gradient accent bar */}
        ...
        {/* Title */}
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-teal-600 dark:group-hover:text-teal-400 transition">
          {title}
        </h3>
    ...
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
      ...
      <section className="text-center mb-16">
        ...
        <p className="mt-4 text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
          Free programming courses with interactive quizzes, code playgrounds,
          and step-by-step visualizations. No signup required.
        </p>
      </section>
...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 ${className}`}>
      {children}
    </div>
  );
}
        ...
        <div className="grid gap-8 sm:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500">
              Free interactive programming courses.
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2">
              <li><FooterLink href="/blog/en/nextjs-ghpages">Next.js Blog</FooterLink></li>
              <li><FooterLink href="/blog/en/algorithms">Algorithms</FooterLink></li>
              <li><FooterLink href="/blog/en/docker101">Docker</FooterLink></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2">
              <li><FooterLink href="https://github.com/yourusername">GitHub</FooterLink></li>
              <li><FooterLink href="https://twitter.com/yourhandle">Twitter</FooterLink></li>
              <li><FooterLink href="https://buymeacoffee.com/yourusername">Buy Me a Coffee</FooterLink></li>
            </ul>
          </div>
        </div>
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href} className="text-sm text-gray-600 dark:text-gray-400 hover:text-teal-600 transition">
      {children}
    </a>
  );
}
