...
[← Chapter 14: Hooks Deep Dive](chapter-14-hooks-deep-dive.md)
...
Most of the time, TypeScript *infers* types — you don't write them:
...
export function Quiz({ question, options, answer, explanation }: QuizProps) {
  ...
  const handleSelect = (index: number) => {  // index is typed
    if (revealed) return;
    setSelected(index);
    setRevealed(true);
  };
...
export function getAllSeries(): SeriesInfo[] {
  ...
  return fs.readdirSync(base, { withFileTypes: true })
    ...
    .map((d): SeriesInfo => ({
      name: d.name,
      slug: d.name,
      chapters: fs.readdirSync(path.join(base, d.name))
...
Sometimes a function works with *any* type:
...
```typescript
interface QuizProps { question: string; options: string[]; }
