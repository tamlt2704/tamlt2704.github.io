...
[← Chapter 7: Finishing Touches](chapter-07-finishing-touches.md) | [Chapter 9: Streaming on Static →](chapter-09-streaming.md)
...
The problem: React hydrates *after* the browser paints. If the user prefers dark mode, they'll see a white flash before JavaScript adds the `dark` class. The fix: inject a tiny script in `<head>` that runs *before* any CSS paints.
...
**Why not just use `useEffect`?** Because `useEffect` runs *after* the component renders. By then, the browser has already painted the white background. This inline script runs synchronously in `<head>`, before any paint happens.
